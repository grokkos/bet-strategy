...
public class GERMANY3 {
    public static void main(String[] args) throws SQLException {
        ...
        try {
            ...
            doc = Jsoup.connect("http://www.forebet.com/en/football-tips-and-predictions-for-germany/3-liga").get();
        } catch (IOException e) {
            e.printStackTrace();
        }
        try {
        ...
        try {
            ...
            doc1 = Jsoup.connect("http://www.forebet.com/en/football-tips-and-predictions-for-germany/3-liga/under-over").get();
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for (Element row : table1.select("tr:gt(1)")) {  // η for εξασφαλιζει οτι με τις αντιστοιχες επαναλήψεις θα περαστούν ολα τα στοιχεία του πινακα στη βαση μας
            ...
            if (td.eq(0).hasText()) {
                ...
                PreparedStatement preparedStmt1 = conn.prepareStatement("update Leagues.Germany3 set ProbUnder = ?, ProbOver = ?, PickGoal = ? , SuccessPickGoal = ? where Teams = ?");
        ...
        try {
            ...
            doc2 = Jsoup.connect("http://www.forebet.com/en/football-tips-and-predictions-for-germany/3-liga/halftime-fulltime").get();
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for (Element row : table2.select("tr:gt(1)")) {  // η for εξασφαλιζει οτι με τις αντιστοιχες επαναλήψεις θα περαστούν ολα τα στοιχεία του πινακα στη βαση μας
            ...
            if(td.eq(0).hasText()){
                ...
                PreparedStatement preparedStmt2 = conn.prepareStatement("update Leagues.Germany3 set HalfProb1 = ?, HalfProbX = ?, HalfProb2 = ?, PickHT = ?, PickFT = ?, SuccessPickHT = ?, SuccessPickFT = ?  where Teams = ?");
        ...
        try {
            ...
            doc3 = Jsoup.connect("http://www.forebet.com/en/football-tips-and-predictions-for-germany/3-liga/bothtoscore").get();
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for (Element row : table3.select("tr:gt(1)")) {  // η for εξασφαλιζει οτι με τις αντιστοιχες επαναλήψεις θα περαστούν ολα τα στοιχεία του πινακα στη βαση μας
            ...
            if(td.eq(0).hasText()){
                ...
                PreparedStatement preparedStmt3 = conn.prepareStatement("update Leagues.Germany3 set NGprob = ?, GGprob = ?, BTSpick = ?, SuccessBTSpick = ?, BTSodds = ? where Teams = ?");
